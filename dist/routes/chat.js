...
const litellm_1 = require("../services/litellm");
const hal_1 = require("../services/hal");
const supabase_1 = require("../services/supabase");
async function chatHandler(req, res) {
    const { message, session_id, model_preference, byok_provider, } = req.body;
    ...
    const sessId = session_id || (0, uuid_1.v4)();
    const start = Date.now();
    let llmResult;
    try {
        llmResult = await (0, litellm_1.callLitellm)(message, model_preference);
    }
    catch (err) {
        console.error('Litellm error:', err);
        return res.status(504).json({ error: 'LLM service failed.' });
    }
    let halResult;
    try {
        halResult = await (0, hal_1.scoreWithHal)(message, llmResult.response_text, llmResult.tokens_used);
    }
    catch (err) {
